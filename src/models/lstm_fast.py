...
class Config(object):
    ...
    hidden_size = 400
    max_epoch = 4
    max_max_epoch = 20  # was 13 originally
    keep_prob = 0.5  # dropout was 1.0 originally
    lr_decay = 0.8
    batch_size = 64
    ...
    def __init__(self, is_training, config, input_):
        ...
        with tf.device("/cpu:0"):
            ...
            inputs = tf.nn.embedding_lookup(self.embedding,
                                            input_.input_data)
            test_inputs = tf.nn.embedding_lookup(self.embedding,
                                                 test_word_in)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Test"):
            ...
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                mtest = RnnlmModel(is_training=False, config=config, input_=test_input)
        # tf.summary.scalar("Training Loss", m.cost)
        # tf.summary.scalar("Learning Rate", m.lr)
