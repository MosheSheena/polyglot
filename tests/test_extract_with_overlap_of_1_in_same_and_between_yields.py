...
class TestGenNoOverlap(unittest.TestCase):
    ...
    def test_empty_file_with_seq_len(self):
        with open('test_files/empty') as f:
            g = extract_x_without_overlap_y_shifted_by_1(opened_file=f, seq_len=2)
            res = list(g)
        self.assertEqual(len(res), 0)

    def test_empty_file_without_seq_len(self):
        with open('test_files/empty') as f:
            g = extract_x_without_overlap_y_shifted_by_1(opened_file=f, seq_len=0)
            res = list(g)
        self.assertEqual(res, [])

    def test_one_line_with_leftover(self):
        with open('test_files/one_line') as f:
            g = extract_x_without_overlap_y_shifted_by_1(opened_file=f, seq_len=4)
            res = list(g)
        self.assertEqual(res,
                         [
    ...
    def test_one_line_without_leftover(self):
        with open('test_files/one_line') as f:
            g = extract_x_without_overlap_y_shifted_by_1(opened_file=f, seq_len=7)
            res = list(g)
        self.assertEqual(res,
                         [
                             (
    ...
    def test_one_line_with_seq_len_of_1(self):
        with open('test_files/one_line') as f:
            g = extract_x_without_overlap_y_shifted_by_1(opened_file=f, seq_len=1)
            res = list(g)
        self.assertEqual(res,
                         [
                             (['WONDER'], ['HOW']), (['HOW'], ['MUCH']),
    ...
    def test_one_line_seq_len_equal_num_words_in_file(self):
        with open('test_files/one_line') as f:
            g = extract_x_without_overlap_y_shifted_by_1(opened_file=f, seq_len=15)
            res = list(g)
        self.assertEqual(res, [])

    def test_one_line_seq_len_bigger_than_words_in_file(self):
        with open('test_files/one_line') as f:
            g = extract_x_without_overlap_y_shifted_by_1(opened_file=f, seq_len=16)
            res = list(g)
        self.assertEqual(res, [])
    ...
    def test_short_lines_without_leftover(self):
        with open('test_files/short_lines') as f:
            g = extract_x_without_overlap_y_shifted_by_1(opened_file=f, seq_len=3)
            res = list(g)
        self.assertEqual(res,
                         [
